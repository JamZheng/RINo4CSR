...
class RINoCSR(nn.Module):
    def __init__(self, config):
        super(RINoCSR, self).__init__()
        ...
        self.protos = InterestProtos(config)
    ...
    def kmeans_item_scl_loss(self, input_items_ids):
        all_item_embs = self.item_embeddings.weight
        input_item_embs = self.item_embeddings(input_items_ids) # [B, embs]
        if self.sim_type == 'cos':
            all_item_embs = F.normalize(all_item_embs, dim=-1)
            input_item_embs = F.normalize(input_item_embs, dim=-1)
        return self.protos.cal_item_cluster_scl(input_items_ids, input_item_embs, all_item_embs)

    def kmeans_aug_forward(self, item_seq, item_seq_len, main_protos_embs,  phase='predict'):
        # get positon embedding 
        item_emb = self.item_embeddings(item_seq)
        if self.sim_type == 'cos':
            item_emb = F.normalize(item_emb, dim=-1) # test

        padding_embs = self.item_embeddings(torch.zeros_like(item_seq, device=item_seq.device))

        item_emb = self.protos.kmeans_gumbel_softmax_drop(item_emb, main_protos_embs, padding_embs)

        return self.item_embs_forward(item_seq, item_emb, item_seq_len, phase=phase)

    def item_embs_forward(self, item_seq, item_emb, item_seq_len, phase='predict'):
        position_ids = torch.arange(item_emb.size(1), dtype=torch.long, device=item_emb.device)        
        position_ids = position_ids.unsqueeze(0).expand(size=(item_emb.size(0), item_emb.size(1)))
        position_embedding = self.position_embedding(position_ids)
    ...
    def get_item_embs_weight(self):
        if self.sim_type == 'cos':
            return F.normalize(self.item_embeddings.weight, dim=-1)
    ...
    def cross_entropy(self, seq_out, poss, negs):
        ...
        pos_logits = torch.sum(pos * seq_emb, -1) / self.cos_tau  # [batch*seq_len]
        neg_logits = torch.sum(neg * seq_emb, -1) / self.cos_tau  # [batch*seq_len]
