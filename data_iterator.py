...
class Data(object):
    def __init__(self, data_name, max_len, logger):
        ...
        train_rate = 1
        if train_rate < 1:
            self.pos_seqs = self.pos_seqs[:ceil(train_rate * len(self.pos_seqs))]
    ...
    def write_renumber(self, path):
        with open(path, 'w') as f:
            for user, seq in enumerate(self.pos_seqs):
                f.write(str(user + 1))
                for item in seq:
                    f.write(' ' + str(item))
                f.write('\n')
    ...
    def get_item_user_dict(self):
        reversed_dict = dict()
        for uid, seq in enumerate(self.pos_seqs):
            for item in seq:
                if item in reversed_dict.keys():
                    reversed_dict[item].append(uid+1)
                else:
                    reversed_dict[item] = [uid+1]
        item_frequency = dict()
        for item in reversed_dict.keys():
            item_frequency[item] = len(reversed_dict[item])
            
        assert(len(item_frequency) == len(reversed_dict))
        return reversed_dict, item_frequency
...
class TrainData(Data):
    def __init__(self, config):
        ...
        self.min_pre_aug_len = config.min_pre_aug_len
